...
- **Compile options**: លក្ខណះសំខាន់របស់ **Kotlin** គឺ `null-safety` ដែលជាវាជួយដោះស្រាយបញ្ហាបានយ៉ាងច្រើននៅពេល `compile time` វាជៀសវាងនូវការ `exception` ដ៏ល្បីគឺ **NullPointerException** នៅពេល `run time` ។ ដើម្បីបន្ថែមនូវមុខងារ `null-safety` នេះបានគឺអ្នកត្រូវបន្ថែមនូវ `-Xjsr305` _compiler_ ដែលមាន _flag_ ជាមួយនឹង `strict` ។
